...
pub struct ResponsesRequest {
    ...
    /// Optional stable key for OpenAI prompt caching. Only serialized
    /// when configured. Endpoints that don't support it ignore it.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_cache_key: Option<String>,
    /// Optional explicit/implicit prompt caching mode. Omitted by
    /// default so the upstream uses its own default behavior.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// OpenAI prompt-caching options. Only serialized when set.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct PromptCacheOptions {
    /// `"implicit"` or `"explicit"`.
    pub mode: String,
    /// Currently only `"30m"` is supported by OpenAI; omit unless
    /// you know the upstream accepts another value.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub ttl: Option<String>,
}

/// Marks the end of a reusable prompt prefix for OpenAI prompt caching.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
